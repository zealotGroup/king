<template>
  <div class="app-container">
    ...
    <el-table :key='table.key' :data="table.list" v-loading="table.loading" border fit highlight-current-row
      ...
      <el-table-column min-width="100px" :label="$t('supplierName')">
        <template slot-scope="scope">
...
<script>
  import { getList, get, add, update, del } from '@/api/jxc/purchase'
  ...
  import { getList as getSupplierList } from '@/api/jxc/supplier'
  ...
  export default {
    ...
    created() {
      this.resetTemp()
      this.resetDialog()
      getSupplierList({ page: 1, limit: -1 }).then((data) => {
